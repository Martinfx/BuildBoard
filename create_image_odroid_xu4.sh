...
UBOOT_MIRROR="https://mirror.applebred.net/armbian-beta/pool/main/l/linux-u-boot-odroidxu4-dev/"
...
DEV="md99"
DATASET="zroot/odroid"
IMG="odroid_xu4-$FREEBSD_VER.img"
...
fetch_uboot() {
        UBOOT_NAME="linux-u-boot-dev-odroidxu4_5.77.190401_armhf"
	DATA="data.tar.xz"
	UBOOT_PATH="uboot"
...
install_uboot() {
	dd if=$UBOOT_PATH/bl1.bin.hardkernel of=$IMG  seek=1 conv=notrunc
	dd if=$UBOOT_PATH/bl2.bin.hardkernel.720k_uboot of=$IMG seek=31 conv=notrunc
	dd if=$UBOOT_PATH/u-boot-dtb.bin of=$IMG seek=63 conv=notrunc
	dd if=$UBOOT_PATH/tzsw.bin.hardkernel of=$IMG seek=1603 conv=notrunc
}
...
install_freebsd() {
	...
	[ -f kernel-dbg.txz ] || fetch $FREEBSD_MIRROR/$FREEBSD_VER/kernel-dbg.txz

	cd $MOUNTPOINT
	tar -xJf $WORK_DIR/base.txz
	tar -xJf $WORK_DIR/kernel.txz
	tar -xJf $WORK_DIR/kernel-dbg.txz
	cd $WORK_DIR
}
