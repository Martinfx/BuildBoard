...
PUB_KEY=""
#UBOOT_MIRROR="https://mirrors.dotsrc.org/armbian-apt/pool/main/l/linux-u-boot-bananapim3-legacy"
UBOOT_MIRROR="https://mirror.applebred.net/armbian-beta/pool/main/l/linux-u-boot-bananapim3-dev"
#FREEBSD_MIRROR="https://download.freebsd.org/ftp/$FREEBSD_BRANCH/arm64"
FREEBSD_MIRROR="https://download.freebsd.org/snapshots/arm64/14.0-CURRENT/"
DEV="md99"
DATASET="zroot/orangepi"
IMG="artik5-$FREEBSD_VER.img"
...
fetch_uboot() {
        #UBOOT_NAME="linux-u-boot-dev-bananapim3_5.53.180722_armhf"
	#DATA="data.tar.xz"
	#UBOOT_PATH="uboot"
...
install_uboot() {
#	dd if=$UBOOT_PATH/u-boot-sunxi-with-spl.bin of=$IMG bs=1k seek=8 conv=notrunc
#	dd if=$UBOOT_PATH/uboot.img of=$IMG seek=16384 conv=notrunc
#	dd if=$UBOOT_PATH/trust.bin of=$IMG seek=24576 conv=notrunc
	wget -c https://rcn-ee.com/repos/git/build-artik/prebuilt/artik5/bl1.bin
	dd if=./bl1.bin of=$IMG seek=1 bs=512
	wget -c https://rcn-ee.com/repos/git/build-artik/prebuilt/artik5/bl2.bin
	dd if=./bl2.bin of=$IMG seek=31 bs=512
	wget -c https://rcn-ee.com/repos/bootloader/artik5/u-boot-artik5-v2012.07-r3.bin
	dd if=./u-boot-artik5-v2012.07-r3.bin of=$IMG seek=63 bs=512
	wget -c https://rcn-ee.com/repos/git/build-artik/prebuilt/artik5/tzsw.bin
	dd if=./tzsw.bin of=$IMG seek=719 bs=512
}
